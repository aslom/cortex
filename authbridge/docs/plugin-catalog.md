...
| `output_usd` | the modelled cost of the generated tokens alone, at the output rate. The other half of the same breakdown, so a per-row UI can show what a response row cost rather than what the exchange cost. Priced from the table, **never** `cost_usd` − `prompt_usd`: that difference mixes a possibly-authoritative total with a modelled half, so it concentrates the table's whole error into the completion figure and can go negative |
